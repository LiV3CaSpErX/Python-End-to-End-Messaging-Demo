...
print("export the keypair to 'serverPrivateKey.der' with AES encryption in binary format")
prikey_in_der=rsakey_pair.export_key(format="DER", passphrase=secret_phrase, pkcs=8,protection="scryptAndAES256-CBC")
try:
    open("private/serverPrivateKey.der","wb").write(prikey_in_der)
...
print("export the public key to 'serverPublicKey.pem' with Base64 format")
try:
    open("public/serverPublicKey.pem","wb").write(pubkey_in_pem)
...
prikey_bytes=open("private/serverPrivateKey.der","rb").read()
...
pubkey_bytes=open("public/serverPublicKey.pem","r").read()
